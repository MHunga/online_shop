...
class ItemProduct extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Hero(
        tag: product.urlPhoto,
        child: FlatButton(
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(0)),
          padding: EdgeInsets.all(0),
          onPressed: (){
            pushNewScreen(context,
                screen: DetailsShoe(product: product, listKey: listKey, listCart: listCart,tag: product.urlPhoto,),
            customPageRoute: PageRouteBuilder(
                pageBuilder: (context,animation, anotherAnimation){
                  return DetailsShoe(product: product,listKey: listKey, listCart: listCart,tag: product.urlPhoto,);
                },
              ...
              Expanded(
                  ...
                  decoration: BoxDecoration(
                    image: DecorationImage(
                      fit: BoxFit.cover,
                      image: CachedNetworkImageProvider(product.urlPhoto)
                    )
                  ),
                  )),
              Expanded(
