...
class CartScreen extends StatefulWidget {
  ...
  bool isFromNav;
  CartScreen({Key? key, this.listKey, this.isFromNav = false})
      : super(key: key);
  @override
...
class CartScreenState extends State<CartScreen> with TickerProviderStateMixin {
  ...
  _mainBody(BuildContext context) {
    return Consumer<CartProvider>(
      builder: (context, cartPrv, child) {
        return cartPrv.listCart.length > 0
            ? Stack(
                children: [
                  AnimatedList(
                      key: widget.isFromNav ? widget.listKey : listKey,
                      padding:
                          EdgeInsets.only(left: 15, right: 15, bottom: 155),
  ...
  _deleteItem(BuildContext context, int index, int id) async {
    ...
    Provider.of<CartProvider>(context, listen: false).deleteCart(
        index: index,
        listKey: listKey,
        listKeyNav: widget.listKey!,
        isNav: widget.isFromNav);
  }
}
