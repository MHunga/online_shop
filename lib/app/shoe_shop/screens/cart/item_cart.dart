...
class ItemCart extends StatefulWidget {
  ...
  const ItemCart(
      {Key? key, this.cart, this.onChange, this.onDelete, this.index})
      : super(key: key);
  @override
...
class _ItemCartState extends State<ItemCart> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animationControllerType = AnimationController(
      vsync: this,
      duration: Duration(milliseconds: 1000 + widget.index! * 10),
    );
    _animationControllerPrice = AnimationController(
      vsync: this,
      duration: Duration(milliseconds: 1200 + widget.index! * 10),
    );
    _animationControllerCal = AnimationController(
      vsync: this,
      duration: Duration(milliseconds: 1400 + widget.index! * 10),
    );
    _animationScale = Tween<double>(begin: 0, end: 1).animate(CurvedAnimation(
        parent: _animationController!,
        curve: Curves.elasticOut,
        reverseCurve: Curves.elasticOut));
    _animationTransformName =
        Tween<Offset>(begin: Offset(200, 0), end: Offset.zero).animate(
            CurvedAnimation(
                parent: _animationControllerName!,
                curve: Curves.fastOutSlowIn));
    _animationTransformType =
        Tween<Offset>(begin: Offset(200, 0), end: Offset.zero).animate(
            CurvedAnimation(
                parent: _animationControllerType!,
                curve: Curves.fastOutSlowIn));
    _animationTransformPrice =
        Tween<Offset>(begin: Offset(200, 0), end: Offset.zero).animate(
            CurvedAnimation(
                parent: _animationControllerPrice!,
                curve: Curves.fastOutSlowIn));
    _animationTransformCal =
        Tween<Offset>(begin: Offset(200, 0), end: Offset.zero).animate(
            CurvedAnimation(
                parent: _animationControllerCal!, curve: Curves.fastOutSlowIn));
                  ...
                  children: [
                    ...
                    InkWell(
                        onTap: () {
                          ...
                          Future.delayed(
                              Duration(milliseconds: 1000 + widget.index! * 10),
                              () {
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          AnimatedSwitcher(
                              ...
                              child: Text(
                                "${widget.cart!.amount}",
                                key: ValueKey(widget.cart!.amount),
                                style: TextStyle(
  ...
  _itemCalculation(
      {BuildContext? context, Color? color, String? item, Function? onTap}) {
    return InkWell(
      onTap: () {
        onTap!();
      },
